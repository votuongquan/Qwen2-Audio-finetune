...
ds_collections = {
    ...
    'aishell2': {'path': './aishell2_eval.jsonl', 'language': 'zh'},
...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, ds,wav_path="/hpc_stor01/home/yangui.fang_sx/workingspace/data/aishell-2/asr/test/my_wav.scp"):
        path = "/hpc_stor01/home/yangui.fang_sx/workingspace/github/Qwen2-Audio/eval_audio/aishell2_eval.jsonl"
        self.datas = open(path).readlines()
        self.wavs = {}
        wav_path = open(wav_path).readlines()
        for line in wav_path:
            id,wav = line.split(" ",1)
            self.wavs[id] = wav.strip()
...
def collate_fn(inputs, processor):
    ...
    input_audios = [ kaldiio.load_mat(dataset.wavs[_])[1].astype(np.float32)  for _ in audio_path]
